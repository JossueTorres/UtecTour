...
class Secciones_controller extends CI_Controller
{
	...
	{        
		$url = 'http://localhost:8080/TUTEC/UtecTourServices/Seccion/listado/';
        //creamos        
		$_param = array(
        ...
        // $url ="http://localhost:8080/TUTEC/UtecTourServices/Recurso/listado/";

        // $postData = array('ddlSec'=>1,'ddlIdm'=>1);
		//create name value pairs seperated by &
		// foreach ($_param as $k => $v) {
		// 	$postData .= $k . '=' . $v . '&';
		// }
		...
		$ua = 'Mozilla/5.0 (Windows; U; Windows NT 5.1; en-US) AppleWebKit/525.13 (KHTML, like Gecko) Chrome/0.A.B.C Safari/525.13';
		$url = 'http://localhost/UtecTourServices/Secciones/listado';
		$ch = curl_init($url);
		curl_setopt($ch, CURLOPT_URL, $url);
		...
		$data['Arrays'] = json_decode($result);
